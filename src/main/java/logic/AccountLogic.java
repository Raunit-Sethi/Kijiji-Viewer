...
public class AccountLogic extends GenericLogic<Account,AccountDAL>{
    ...
    public Account createEntity(Map<String, String[]> parameterMap) {
        ...
        if(displayName==null || displayName.isEmpty()){throw new ValidationException("Display Name cannot be null");}
        if (displayName.length() > 45) {throw new ValidationException("Display Name cannot be null");}
        account.setDisplayName(displayName);
        
        if(user==null || user.isEmpty()){throw new ValidationException("User cannot be null");}
        if (user.length() > 45) {throw new ValidationException("User cannot be null");}
        account.setUser(user);
        
        if(password==null || password.isEmpty()){throw new ValidationException("Password cannot be null");}
        if (password.length() > 45) {throw new ValidationException("Password cannot be null");}
